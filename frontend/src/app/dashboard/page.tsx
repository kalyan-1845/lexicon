...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen bg-[var(--theme-bg)] text-[var(--theme-text)] selection:bg-indigo-500/30">
      <Sidebar 
        ...
        <div className="max-w-6xl mx-auto p-12 relative z-10">
          {isLoading ? (
            ...
            <div className="animate-in fade-in slide-in-from-left-4 duration-700">
              ...
              <h1 className="text-3xl font-extrabold tracking-tight mb-2">Welcome back, <span className="text-[var(--theme-text-muted)]">Kalyan</span></h1>
              <p className="text-[var(--theme-text-muted)] font-medium text-sm">Your agentic workspace is ready for deep analysis.</p>
            </div>
            
            <div className="flex flex-col items-end gap-3">
              <div className="flex gap-4">
                  <div className="px-4 py-2 bg-white/[0.03] border border-[var(--theme-border)] rounded-xl flex flex-col items-end animate-in fade-in slide-in-from-right-4 duration-700">
                    <span className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Global Documents</span>
                    <span className="text-xl font-extrabold text-[var(--theme-text)]">142</span>
                  </div>
                  <div className="px-4 py-2 bg-white/[0.03] border border-[var(--theme-border)] rounded-xl flex flex-col items-end animate-in fade-in slide-in-from-right-4 duration-700 delay-100">
              ...
              <div className="flex items-center gap-3 bg-[var(--theme-surface)]/80 border border-[var(--theme-border)] rounded-full px-4 py-1.5 animate-in fade-in slide-in-from-right-4 duration-700 delay-200">
                <span className="text-[10px] font-bold text-[var(--theme-text-muted)] uppercase tracking-widest">Live Now:</span>
                <div className="flex -space-x-2">
                  {collaborators.map((c) => (
                    <div key={c.id} title={c.status} className={`w-8 h-8 rounded-full ${c.color} border-2 border-[#09090b] flex items-center justify-center text-[10px] font-bold shadow-lg cursor-pointer hover:-translate-y-1 transition-transform relative group`}>
                      {c.avatar}
                      {/* Tooltip */}
                      <div className="absolute top-10 w-max px-2 py-1 bg-[var(--theme-border)] backdrop-blur-md rounded border border-[var(--theme-border)] text-[9px] opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                        {c.name} - {c.status}
                      </div>
                    </div>
                  ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            {workspaces.map((ws, i) => (
              <div 
                ...
                className="group relative p-8 bg-[var(--theme-surface)]/80 backdrop-blur-xl border border-[var(--theme-border)] rounded-2xl hover:border-[var(--theme-border)] transition-all cursor-pointer animate-in fade-in slide-in-from-bottom-4 duration-700 fill-mode-both"
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-start mb-6">
                    <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${ws.color} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform`}>
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-[var(--theme-text)]">
                    ...
                    <div className="flex flex-col items-end">
                      ...
                      <span className="text-[10px] font-bold text-[var(--theme-text-muted)]">{ws.lastActive}</span>
                    </div>
                  </div>
                  <h3 className="font-bold text-lg mb-2 group-hover:text-[var(--theme-text)] transition-colors">{ws.name}</h3>
                  <div className="flex items-center gap-3">
                    <div className="h-1 flex-1 bg-white/[0.03] rounded-full overflow-hidden">
                      <div className="h-full bg-[var(--theme-border)] w-2/3 group-hover:bg-white/20 transition-all" />
                    </div>
                    <span className="text-[10px] font-extrabold text-[var(--theme-text-muted)] uppercase tracking-tighter">{ws.docs} Sources</span>
                  </div>
                </div>
              </div>
            ))}
            ...
            <button 
              ...
              className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-[var(--theme-border)] rounded-2xl hover:bg-white/[0.02] hover:border-[var(--theme-border)] transition-all group animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300 fill-mode-both"
            >
              <div className="w-12 h-12 rounded-full border border-dashed border-[var(--theme-border)] flex items-center justify-center mb-4 group-hover:scale-110 group-hover:border-[var(--theme-border)] transition-all">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-gray-600 group-hover:text-[var(--theme-text-muted)] transition-colors">
          ...
          <section className="animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-500 fill-mode-both">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-[10px] font-extrabold text-[var(--theme-text-muted)] uppercase tracking-[0.2em]">Real-Time Collaboration Pulse</h2>
            </div>
            
            <div className="bg-[var(--theme-surface)]/50 backdrop-blur-md border border-[var(--theme-border)] rounded-2xl p-6 relative overflow-hidden">
              ...
              <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                {activities.map((activity) => (
                  <div key={activity.id} className="flex items-start gap-4 p-3 hover:bg-white/[0.02] rounded-xl transition-colors border border-transparent hover:border-[var(--theme-border)] animate-in fade-in slide-in-from-top-2">
                     ...
                     <div className="flex-1 min-w-0">
                       <p className="text-sm font-medium text-[var(--theme-text)]">
                         <span className="font-bold text-[var(--theme-text)]">{activity.user}</span> {activity.action} <span className="font-bold text-indigo-400 cursor-pointer hover:underline">{activity.target}</span>
                       </p>
                       <span className="text-xs text-[var(--theme-text-muted)] font-medium">{activity.time}</span>
                     </div>
                  </div>
                ))}
              </div>
